...
export default function Header() {
  ...
  // Full label for desktop, short label for mobile
  const weekLabelFull = currentWeek
    ? `${format(new Date(currentWeek.start_date + 'T00:00:00'), 'MMM d')} – ${format(new Date(currentWeek.end_date + 'T00:00:00'), 'MMM d, yyyy')}`
    : '—'
  const weekLabelShort = currentWeek
    ? `${format(new Date(currentWeek.start_date + 'T00:00:00'), 'MMM d')}–${format(new Date(currentWeek.end_date + 'T00:00:00'), 'd')}`
    : '—'
      ...
      <header style={{
        ...
        <div style={{ position: 'relative', flexShrink: 1, minWidth: 0 }}>
          ...
          >
            ...
            <span className="wk-label-full">{weekLabelFull}</span>
            <span className="wk-label-short">{weekLabelShort}</span>
            ...
            <div style={{
              ...
              {state.weeks.map(week => {
                ...
                return (
                  ...
                  >
                    {label}
                    {week.status === 'closed' && <span style={{ marginLeft: 8, color: 'var(--color-text-muted)', fontSize: 9 }}>CLOSED</span>}
                    {isCurrent && <span style={{ marginLeft: 8, color: 'var(--color-accent)', fontSize: 9 }}>CURRENT</span>}
                  </button>
                )
              })}
            </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
          ...
          {unsortedCount > 0 && (
            <div className="header-intake-badge" style={{
              fontFamily: 'var(--font-mono)',
              fontSize: '10px',
          ...
          {currentWeek?.status !== 'closed' && (
            ...
            >
              <span className="new-week-full">New Week</span>
              <span className="new-week-short">+ WK</span>
            </button>
          )}
        </div>

        <style>{`
          .wk-label-short { display: none; }
          .new-week-short { display: none; }
          @media (max-width: 520px) {
            .wk-label-full { display: none; }
            .wk-label-short { display: inline; }
            .new-week-full { display: none; }
            .new-week-short { display: inline; }
            .header-pdf-btn { display: none; }
          }
        `}</style>
      </header>
